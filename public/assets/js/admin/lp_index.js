...
function doDelete(id,name){
  if(confirm("Apakah Anda yakin menghapus produk hukum '"+name+"'? Aksi ini tidak dapat dibatalkan.")){
    axios.post(baseUrl+'/api/lp/post-delete/'+id, {}, apiHeaders)
...
$(function () {
  function getData(){
    ...
    let url = baseUrl+'/api/get-lp-listfull'
    ...
    .then(function (response) {
      ...
      if(response.data.status) {
          var table = $('#data-list').DataTable({ // https://datatables.net/manual/data/
            ...
            columns: [ 
              { data: 'id'},
              { data: 'legal_product_type' },
              { data: 'year' },
              { data: 'number' },
              { data: 'status' },
              { data: null, render: function ( data, type, row ) { 
                  return '<a href="'+baseUrl+'/admin-jdih-katkab/lp/edit/'+data.id+'" target="_blank">'+data.title+'</a>';
                } 
              },
              { data: null, render: function ( data, type, row ) {
                  return '<a href="'+baseUrl+data.file_main+'" target="_blank">lihat</a>';
                } 
              },
              { data: null, render: function ( data, type, row ) { 
