  ...
  if(confirm("Apakah Anda yakin menghapus dokumen '"+name+"'? Aksi ini tidak dapat dibatalkan.")){
    ...
    .then(function (response) {
      ...
      if(response.status == 200 && response.data.status) {
        ...
        window.location = baseUrl+'/admin-jdih-katkab/cs';
      }else{
        Swal.fire({
          icon: 'warning',
          width: 600,
...
$(function () {
  function getData(){
    ...
    let url = baseUrl+'/api/get-cs-listfull'
          ...
          var table = $('#data-list').DataTable({ // https://datatables.net/manual/data/
            ...
            columns: [ 
              ...
              { data: null, render: function ( data, type, row ) { 
                  return '<a href="'+baseUrl+'/admin-jdih-katkab/cs/edit/'+data.id+'" target="_blank">'+data.title+'</a>';
                } 
              },
              { data: 'date_register' },
              { data: 'plaintiff' },
