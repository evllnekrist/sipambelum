...
$(function(){
  ...
  $("#btn-submit-add").click(function(e){
    ...
    if (!form.checkValidity()) {
      ...
      // }
      axios.post(baseUrl+'/api/gallery/post-add', formData, apiHeaders)
  ...
  $("#btn-submit-edit").click(function(e){
    ...
    if (!form.checkValidity()) {
      ...
      //   console.log('»', key, value)
      // }
      axios.post(baseUrl+'/api/gallery/post-edit', formData, apiHeaders)
      .then(function (response) {
        console.log('response..',response);
        if(response.status == 200 && response.data.status) {
          Swal.fire({
  ...
  $('.media-type').on('change', function(event) {
    ...
    if(value == 'img'){
      $('#media_main_img').show()
      $('#media_main_video').hide()
    }else if(value == 'video'){
      $('#media_main_img').hide()
      $('#media_main_video').show()
    }
  });
